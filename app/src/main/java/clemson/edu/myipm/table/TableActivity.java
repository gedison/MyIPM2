package clemson.edu.myipm.table;
...
import clemson.edu.myipm.database.dao.AffectionActiveDAO;
import clemson.edu.myipm.database.dao.AffectionDAO;
import clemson.edu.myipm.database.dao.AffectionSelectionDAO;
import clemson.edu.myipm.database.dao.AffectionTradeDAO;
import clemson.edu.myipm.database.dao.FruitDAO;
import clemson.edu.myipm.database.dao.MainScreenDAO;
import clemson.edu.myipm.database.dao.TableEntry;
...
//        }
    ...
    public void getData(){
        if(tableType == 0){
            ...
            AffectionTradeDAO affectionTradeDAO = new AffectionTradeDAO(this);
            if(activeId!=-1)
                data = affectionTradeDAO.getAffectionTradeDataWithTypeWithAffectionWithActiveId(typeID, SharedPreferencesHelper.getAffectionId(this), activeId);

            else
                data = affectionTradeDAO.getAffectionTradeDataWithTypeWithAffection(typeID, SharedPreferencesHelper.getAffectionId(this));
        }
    ...
    public void showHelpPopup(){
        ...
        builder.setMessage(R.string.table_help_contents).setTitle(R.string.table_help_title)
                .setNegativeButton("Don't show again", new DialogInterface.OnClickListener() {

                    public void onClick(DialogInterface dialogInterface, int i) {
                        SharedPreferencesHelper.setTableHelp(getApplicationContext(), false);
                    }
                })
